...
```bash
# 必须按顺序执行，全部通过才能继续 (基于项目实际scripts)
npm run type-check   # ❌ 任何类型错误 = 立即停止
npm run lint         # ❌ 任何lint错误 = 立即停止  
npm run test:coverage # ❌ 测试覆盖率<80% = 立即停止
npm run build        # ❌ 任何构建错误 = 立即停止
npm run dev          # ❌ 运行时错误 = 立即停止
npm run stylelint    # ❌ 样式规范错误 = 立即停止 (项目特有)
```
...
```json
// tsconfig.json - 强制配置 (与项目实际配置同步)
{
  "compilerOptions": {
    "target": "ES2020",               // ✅ 与项目一致
    "lib": ["ES2020", "DOM", "DOM.Iterable"], // ✅ 与项目一致
    "module": "ESNext",               // ✅ 与项目一致
    "moduleResolution": "bundler",    // ✅ 与项目一致
    "strict": true,                   // ✅ 必须启用
    ...
    "skipLibCheck": true,            // ✅ 与项目一致
    "allowSyntheticDefaultImports": true, // ✅ 与项目一致
    "resolveJsonModule": true,       // ✅ 与项目一致
    "isolatedModules": true,         // ✅ Vite要求
    "useDefineForClassFields": true, // ✅ 现代类字段语法
...
const user: UserInfo = {
  id: "123",                          // ✅ 双引号 (与.prettierrc.json一致)
  name: "John",                       // ✅ 双引号
  email: "john@example.com",          // ✅ 双引号
  createdAt: new Date(),              // ✅ 尾随逗号
}
...
### 强制测试覆盖率 (基于Vitest 2.1.8配置)
```typescript
// ✅ 必须达到的覆盖率指标 (与vitest.config.ts一致)
// 函数覆盖率: >80%  (项目标准)
// 行覆盖率: >80%    (项目标准)
// 分支覆盖率: >80%  (项目标准)
// 语句覆盖率: >80%  (项目标准)
...
### 强制性能标准 (基于Vue 3.5.13 + Vite 6.0.5)
```typescript
// ✅ 必须满足的性能指标 (现代Web标准)
...
// ✅ 强制使用：Vue 3性能优化技术
// 1. Vue组件懒加载 (defineAsyncComponent)
...
// 5. Element Plus按需导入 (减少包大小)
import { ElButton, ElTable } from "element-plus"
```

## 🚨 自动质量检查

### 代码提交前强制检查 (基于Husky + lint-staged)
```bash
#// Git pre-commit钩子自动执行 (与package.json scripts一致)
1. TypeScript类型检查 (npm run type-check)
2. ESLint规范检查 (npm run lint)
3. Prettier格式化 (npm run format)
4. 样式规范检查 (npm run stylelint)
5. 单元测试执行 (npm run test:coverage)
6. 构建验证 (npm run build)
7. E2E测试 (npm run cypress:run) [可选]
```
...
interface QualityGates {
  buildSuccess: true,           // ✅ Vite 6.0.5构建必须成功
  typeCheckPass: true,          // ✅ TypeScript 8.41.0类型检查必须通过
  lintErrors: 0,                // ✅ ESLint错误必须为0
  stylelintErrors: 0,           // ✅ Stylelint错误必须为0 (项目特有)
  lintWarnings: 0,              // ✅ ESLint警告必须为0
  testCoverage: ">=80%",        // ✅ Vitest测试覆盖率≥80%
  testsPassing: true,           // ✅ 所有测试必须通过
  duplicatedCode: "<5%",        // ✅ 重复代码<5%
  complexityScore: "<10",       // ✅ 圈复杂度<10
  performanceScore: ">90",      // ✅ Lighthouse性能分数>90
  accessibilityScore: ">90",    // ✅ 可访问性分数>90
  bundleSize: "<3MB",           // ✅ 打包大小<3MB
  noConsoleLog: true,           // ✅ 生产代码禁止console.log
  noDebugger: true,             // ✅ 生产代码禁止debugger
  noTodoComments: true          // ✅ 禁止TODO注释进入主分支
}
```
