...
describe("AdvancedForm E2E Tests", () => {
  ...
  const mockInitialValues = {
    ...
    preferences: [{ category: "tech", priority: 8 }],
    agreement: false,
  }
  ...
  it("should handle dynamic array fields with add/remove functionality", () => {
    ...
    cy.get('.array-item [data-field="category"] .el-select .el-select__tags-text').should(
      "contain",
      "设计",
    )
  })
  ...
  it("should enforce array constraints (min/max items)", () => {
    const configWithConstraints = {
      ...
      fields: mockFormConfig.fields.map((field) =>
        field.name === "preferences" ? { ...field, minItems: 2, maxItems: 3 } : field,
      ),
    }
    ...
    cy.get(".form-submit-button").click()
    cy.get('[data-field="preferences"] .field-error').should("contain", "至少需要2项")
  ...
  it("should handle complex enterprise scenarios", () => {
    ...
    cy.get(".form-submit-button").click()
    cy.get('.array-item:first-child [data-field="endTime"] .field-error').should(
      "contain",
      "结束时间必须晚于开始时间",
    )
  ...
  it("should handle performance with large forms", () => {
    ...
    cy.measurePerformance(() => {
      for (let i = 0; i < 10; i++) {
        cy.get(`[data-field="field_${i}"] input, [data-field="field_${i}"] textarea`).type(
          `value_${i}`,
          { force: true },
        )
      }
