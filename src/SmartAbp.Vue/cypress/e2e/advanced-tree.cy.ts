...
describe('AdvancedTree E2E Tests', () => {
  const mockTreeData = [
    {
      ...
      children: [
        ...
        },
        {
          ...
          expanded: false,
          children: [
            { id: '1-3-1', label: '孙八 - 测试工程师', icon: 'user', type: 'employee' }
          ]
        }
      ]
    },
  ...
  const mockLargeTreeData = Array.from({ length: 1000 }, (_, i) => ({
    ...
    expanded: false,
    children: i % 5 === 0 ? Array.from({ length: Math.floor(Math.random() * 10) + 1 }, (_, j) => ({
      id: `node-${i}-${j}`,
      label: `子节点 ${i}-${j}`,
      icon: 'file'
    })) : undefined
  ...
  it('should handle performance with extremely large trees', () => {
    const performanceTestData = Array.from({ length: 50000 }, (_, i) => ({
      ...
      children: i % 20 === 0 ? Array.from({ length: 5 }, (_, j) => ({
        id: `perf-child-${i}-${j}`,
        label: `Child ${j}`,
        icon: 'file'
      })) : undefined
