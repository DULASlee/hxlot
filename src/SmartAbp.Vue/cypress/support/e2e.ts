...
Cypress.Commands.add('simulateSecurityAlert', (alertType: string = 'HighRiskPermissionAccess') => {
  ...
  cy.window().then((win) => {
    win.dispatchEvent(new CustomEvent('security-alert', {
      detail: {
        id: `alert-${Date.now()}`,
        type: alertType,
        severity: 'High',
...
Cypress.Commands.add('verifyPermissionAccess', (permission: string, shouldHaveAccess: boolean = true) => {
...
Cypress.Commands.add('interceptSecurityAPIs', () => {
  cy.intercept('GET', '/api/security/dashboard/metrics', { fixture: 'security-metrics.json' }).as('getMetrics')
  cy.intercept('GET', '/api/security/alerts', { fixture: 'security-alerts.json' }).as('getAlerts')
  cy.intercept('GET', '/api/security/compliance', { fixture: 'compliance-data.json' }).as('getCompliance')
  cy.intercept('GET', '/api/security/behaviors', { fixture: 'abnormal-behaviors.json' }).as('getBehaviors')
  cy.intercept('POST', '/api/security/alerts/*/acknowledge', { success: true }).as('acknowledgeAlert')
