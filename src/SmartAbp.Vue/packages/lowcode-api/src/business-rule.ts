...
import type { PagedResultDto } from './types/paged'
...
export class BusinessRuleApi {
    ...
    async getList(input: GetBusinessRulesInput): Promise<PagedResultDto<BusinessRuleDto>> {
        return await get<PagedResultDto<BusinessRuleDto>>(this.baseUrl, input)
    }
    ...
    async get(id: string): Promise<BusinessRuleDto> {
        return await get<BusinessRuleDto>(`${this.baseUrl}/${id}`)
    }
    ...
    async create(input: CreateBusinessRuleDto): Promise<BusinessRuleDto> {
        return await post<BusinessRuleDto>(this.baseUrl, input)
    }
    ...
    async update(id: string, input: UpdateBusinessRuleDto): Promise<BusinessRuleDto> {
        return await put<BusinessRuleDto>(`${this.baseUrl}/${id}`, input)
    }
    ...
    async delete(id: string): Promise<void> {
        return await del(`${this.baseUrl}/${id}`)
    }
    ...
    async executeRules(input: ExecuteBusinessRuleDto): Promise<BusinessRuleExecutionResultDto[]> {
        return await post<BusinessRuleExecutionResultDto[]>(`${this.baseUrl}/execute`, input)
    }
    ...
    async validateRule(id: string): Promise<BusinessRuleValidationResultDto> {
        return await post<BusinessRuleValidationResultDto>(`${this.baseUrl}/${id}/validate`)
    }
    ...
    async validateAllRules(): Promise<BusinessRuleValidationResultDto[]> {
        return await post<BusinessRuleValidationResultDto[]>(`${this.baseUrl}/validate-all`)
    }
    ...
    async getStats(): Promise<BusinessRuleStatsDto> {
        return await get<BusinessRuleStatsDto>(`${this.baseUrl}/stats`)
    }
    ...
    async getAvailableEntities(): Promise<EntityDefinitionDto[]> {
        return await get<EntityDefinitionDto[]>(`${this.baseUrl}/entities`)
    }
    ...
    async getEntityFields(entityName: string): Promise<EntityFieldDto[]> {
        return await get<EntityFieldDto[]>(`${this.baseUrl}/entities/${entityName}/fields`)
    }
    ...
    async batchUpdateStatus(ruleIds: string[], isActive: boolean): Promise<void> {
        return await put(`${this.baseUrl}/batch-status`, {
            ruleIds,
            isActive
        })
    }
    ...
    async duplicateRule(id: string): Promise<BusinessRuleDto> {
        return await post<BusinessRuleDto>(`${this.baseUrl}/${id}/duplicate`)
    }
    ...
    async batchDelete(ruleIds: string[]): Promise<void> {
        return await del(`${this.baseUrl}/batch`, {
            data: { ruleIds }
        })
    }
    ...
    async exportRules(ruleIds: string[]): Promise<ExportRulesResultDto> {
        return await post<ExportRulesResultDto>(`${this.baseUrl}/export`, ruleIds)
    }
    ...
    async importRules(input: ImportRulesInputDto): Promise<ImportRulesResultDto> {
        return await post<ImportRulesResultDto>(`${this.baseUrl}/import`, input)
    }
}
