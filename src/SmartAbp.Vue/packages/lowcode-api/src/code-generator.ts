 ...
 * 
 * 注意：开发环境使用 http-proxy-middleware 代理（vite.config.ts 中配置）
 * 已解决浏览器 POST JSON 请求通过代理失败的问题
 */
export const codeGeneratorApi: CodeGeneratorApi = {
  ...
  async introspectDatabase(req: any): Promise<any> {
    ...
    try {
      // 🔥 使用longTimeoutHttp（120秒超时）确保大型数据库扫描不会超时
  ...
  async testDatabaseConnection(connection: {
    ...
    return http.post<{
      ...
      tableCount?: number;
      tables?: string[];
    }>('/api/code-generator/test-connection', {
