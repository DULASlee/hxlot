...
export function useCodeGenerationProgress() {
  ...
  const updateProgress = (sessionId: string, update: Partial<CodeGenerationProgress>): void => {
    try {
      ...
      console.log(
        `📈 Updated progress for session ${sessionId}: ${session.percentage}% (${session.generatedFiles}/${session.totalFiles})`,
      )
  ...
  const markFileCompleted = (sessionId: string, fileName: string): void => {
    try {
      ...
      console.log(
        `✅ File completed: ${fileName} (${session.generatedFiles}/${session.totalFiles})`,
      )
    } catch (error) {
