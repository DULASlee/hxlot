...
export function useDragDrop() {
   ...
   * 结束拖拽
   */
  const endDrag = (_event?: DragEvent): void => {
    try {
  ...
  const updateValidDropZones = (data: DragData): void => {
    try {
      ...
      console.log(
        `🎯 Found ${dragState.validDropZones.length} valid drop zones for type: ${data.type}`,
      )
    } catch (error) {
  ...
  const updateDropZonePosition = (zoneId: string): void => {
    try {
      ...
      const rect = zone.element.getBoundingClientRect()
      console.log(
        `📍 Updated position for drop zone ${zone.name}: (${rect.left}, ${rect.top}) - (${rect.right}, ${rect.bottom})`,
      )
    } catch (error) {
