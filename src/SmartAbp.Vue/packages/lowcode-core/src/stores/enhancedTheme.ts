...
export const useEnhancedThemeStore = defineStore("enhancedTheme", () => {
  ...
  const snapshots = ref<ThemeSnapshot[]>([])
  const isUpdating = ref(false)

  // 批量更新队列
  let updateQueue: Array<{ key: string; value: string }> = []
  ...
  const switchTheme = (themeId: string) => {
    ...
    logger.info(`Switched to theme: ${THEME_PRESETS[themeId as keyof typeof THEME_PRESETS]}`)
  }
  ...
  // 批量更新主题变量（性能优化）
  const batchUpdateTheme = (updates: Record<string, string>) => {
  ...
  const createSnapshot = (name?: string) => {
    const snapshot: ThemeSnapshot = {
      ...
      description: `基于${THEME_PRESETS[currentTheme.value as keyof typeof THEME_PRESETS]}主题创建`
    }
  ...
  // 导入主题配置
  const importTheme = (file: File): Promise<boolean> => {
  ...
  watch(currentTheme, (newTheme) => {
    applyTheme()
    logger.info(`Theme switched to: ${THEME_PRESETS[newTheme as keyof typeof THEME_PRESETS]}`)
  })
