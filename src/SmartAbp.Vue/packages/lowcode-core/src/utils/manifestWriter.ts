...
export class SmartAbpManifestWriter {
  ...
  async generateFiles(
    ...
    try {
      ...
      console.log(
        `✅ Successfully generated ${files.length} manifest files for module: ${metadata.name}`,
      )
  ...
  private generateEntityManifest(metadata: ModuleMetadata): any {
    try {
      ...
      // 验证实体定义
      metadata.entities.forEach((entity: any, index: number) => {
        if (!entity.name?.trim()) {
        ...
        // 验证属性定义
        entity.properties.forEach((property: any, propIndex: number) => {
          ...
          if (!validTypes.includes(property.type.toLowerCase())) {
            throw new Error(
              `Property '${property.name}' in entity '${entity.name}' has invalid type: ${property.type}. Valid types are: ${validTypes.join(", ")}`,
            )
          }
        })
      })
  ...
  private generateStore(metadata: ModuleMetadata): any {
    try {
      ...
      const config = {
        ...
        actions: {
          async fetchList({ commit, state }: any, params: any) {
          ...
          async create({ dispatch }: any, item: any) {
          ...
          async update({ dispatch }: any, item: any) {
          ...
          async delete({ dispatch }: any, id: any) {
    ...
    try {
      ...
      menus.forEach((menu, index) => {
        ...
        if (!/^[a-zA-Z][a-zA-Z0-9-]*$/.test(menu.id)) {
          throw new Error(
            `Invalid menu ID: ${menu.id}. Menu ID must start with a letter and contain only letters, numbers, and hyphens`,
          )
        }
    ...
    try {
      ...
      modules.forEach((module, index) => {
        ...
        if (!/^[a-zA-Z][a-zA-Z0-9]*$/.test(module.name)) {
          throw new Error(
            `Invalid module name: ${module.name}. Module name must start with a letter and contain only letters and numbers`,
          )
        }
      })
