...
/**
 * Enhanced Template Search Script (CommonJS) with comprehensive error handling and performance optimization
 * 模板搜索脚本（CommonJS）- 包含性能监控和缓存机制
 */
...
const logInfo = (operation, message, context = {}) => {
  console.log(`[TemplateSearcher] ${operation}:`, {
    message,
    context,
    timestamp: new Date().toISOString()
  });
 ...
 */
function getCacheKey(searchType, query, options) {
  try {
    const optionsHash = Buffer.from(JSON.stringify(options)).toString('base64');
    return `${searchType}:${query || 'all'}:${optionsHash}`;
  } catch (error) {
...
const safeParseJSON = (content, filePath, operation) => {
  try {
    ...
    const errorObj = new Error(`Invalid JSON in ${filePath}: ${error.message}`);
    errorObj.code = 'JSON_PARSE_ERROR';
    errorObj.operation = operation;
...
/**
 * Enhanced templates directory resolution with error handling and performance optimization
 */
function resolveTemplatesDir(startDir) {
      ...
      try {
        if (safeFileExists(candidate, 'resolveTemplatesDir')) {
          ...
          if (stat.isDirectory()) {
            ...
            logInfo('resolveTemplatesDir', 'Found templates directory', {
              path: candidate,
        ...
        logWarning('resolveTemplatesDir', 'Failed to check candidate directory', {
          path: candidate,
          error: error.message
        });
      }
  ...
  const duration = performance.now() - startTime;
  logWarning('resolveTemplatesDir', 'Using fallback templates directory', {
    path: fallback,
    duration: `${duration.toFixed(2)}ms`
  });
  return fallback;
...
/**
 * Enhanced TemplateSearcher class with comprehensive error handling and performance optimization
 */
class TemplateSearcher {
  constructor(options = {}) {
    try {
      ...
      logInfo('constructor', 'TemplateSearcher initialized', {
  ...
  /**
   * Enhanced initialization with comprehensive error handling and performance monitoring
   */
  async init() {
    try {
      const startTime = performance.now();
      ...
      if (!safeFileExists(this.indexPath, 'init')) {
        const error = new Error(`模板索引文件不存在: ${this.indexPath}。请先运行: npm run template:index`);
        error.code = 'INDEX_FILE_NOT_FOUND';
      ...
      if (!this.index || typeof this.index !== 'object') {
        const error = new Error('Invalid index structure');
        error.code = 'INVALID_INDEX_STRUCTURE';
        error.operation = 'init';
        throw error;
      }
      
      if (!Array.isArray(this.index.templates)) {
        const error = new Error('Index templates must be an array');
        error.code = 'INVALID_INDEX_TEMPLATES';
        error.operation = 'init';
        throw error;
      }
      ...
      logInfo('init', 'Template index loaded successfully', {
      ...
      if (error.code === 'INDEX_FILE_NOT_FOUND') {
  ...
  /**
   * Enhanced search with comprehensive error handling, caching, and performance monitoring
   */
  search(query, options = {}) {
    try {
      ...
      if (!this.index) {
        const error = new Error('模板索引未初始化');
        error.code = 'INDEX_NOT_INITIALIZED';
      ...
      if (query !== null && query !== undefined) {
        if (typeof query !== 'string') {
          const error = new Error('查询参数必须是字符串');
      ...
      if (options && typeof options !== 'object') {
        const error = new Error('选项参数必须是对象');
      ...
      // Check cache
      const cacheKey = this.options.enableCache ? getCacheKey('search', query, searchOptions) : null;
      const cachedResult = cacheKey ? getCacheEntry(cacheKey) : null;
      
      if (cachedResult) {
        const duration = performance.now() - startTime;
        logInfo('search', 'Search completed (cached)', {
          query: query || 'all',
      ...
      const results = this.index.templates.filter((template) => {
        try {
          ...
          // Filter by tags
          if (searchOptions.tags && Array.isArray(searchOptions.tags) && searchOptions.tags.length > 0) {
            ...
            const hasTag = searchOptions.tags.some((tag) => {
              ...
              return template.tags.some((templateTag) => {
                if (typeof templateTag !== 'string') return false;
      ...
      // Cache the result
      if (cacheKey) {
        setCacheEntry(cacheKey, limitedResults);
      }
      ...
      if (this.options.enablePerformanceMonitoring) {
        ...
        PERFORMANCE_METRICS.searchTypes.text++;
  ...
  /**
   * Enhanced AI trigger search with comprehensive error handling and performance monitoring
   */
  searchByTrigger(trigger) {
    try {
      ...
      if (!this.index) {
        const error = new Error('模板索引未初始化');
        error.code = 'INDEX_NOT_INITIALIZED';
        error.operation = 'searchByTrigger';
        throw error;
      }

      if (typeof trigger !== 'string') {
        const error = new Error('触发词必须是字符串');
        error.code = 'INVALID_TRIGGER_TYPE';
        error.operation = 'searchByTrigger';
        throw error;
      }
  ...
  getRecommendations(context = {}) {
    try {
      ...
      if (!this.index) {
        const error = new Error('模板索引未初始化');
        error.code = 'INDEX_NOT_INITIALIZED';
        error.operation = 'getRecommendations';
        throw error;
      }

      if (context && typeof context !== 'object') {
        const error = new Error('上下文参数必须是对象');
        error.code = 'INVALID_CONTEXT_TYPE';
        error.operation = 'getRecommendations';
        throw error;
      }
      ...
      if (context.entityType && typeof context.entityType === 'string') {
      ...
      if (context.framework && typeof context.framework === 'string') {
        candidates = candidates.filter((t) => {
          try {
            ...
            const hasCategory = t.category && typeof t.category === 'string' &&
  ...
  showTemplate(templateId) {
    try {
      ...
      if (!this.index) {
        const error = new Error('模板索引未初始化');
        error.code = 'INDEX_NOT_INITIALIZED';
        error.operation = 'showTemplate';
        throw error;
      }

      if (typeof templateId !== 'string') {
        const error = new Error('模板ID必须是字符串');
        error.code = 'INVALID_TEMPLATE_ID_TYPE';
        error.operation = 'showTemplate';
        throw error;
      }
      ...
      const template = this.index.templates.find((t) => {
        try {
          return t && typeof t === 'object' && t.id === templateId;
        } catch (findError) {
        ...
        template.parameters.forEach((param, index) => {
          try {
            if (!param || typeof param !== 'object') {
              logWarning('showTemplate.parameters', 'Invalid parameter object', {
  ...
  listCategories() {
    try {
      ...
      if (!this.index) {
        const error = new Error('模板索引未初始化');
        error.code = 'INDEX_NOT_INITIALIZED';
        error.operation = 'listCategories';
        throw error;
      }
      ...
      Object.entries(this.index.categories).forEach(([main, subs]) => {
        try {
          if (typeof main !== 'string') {
            logWarning('listCategories.main', 'Invalid main category name', { main });
            return;
          }
          ...
          if (typeof subs === 'object' && subs !== null) {
            Object.entries(subs).forEach(([sub, desc]) => {
              try {
                if (typeof sub !== 'string') {
                  logWarning('listCategories.sub', 'Invalid subcategory name', { sub });
                  return;
                }
                
                if (typeof desc !== 'string' && typeof desc !== 'undefined') {
                  logWarning('listCategories.desc', 'Invalid category description', { desc });
                  return;
                }
                
                const count = this.index.templates.filter((t) => {
                  try {
                    return t && typeof t === 'object' && t.category === `${main}/${sub}`;
                  } catch (countError) {
                ...
                console.log(`  - ${sub}: ${desc || 'No description'} (${count} 个模板)`);
              } catch (subError) {
            ...
            logWarning('listCategories.subs', 'Invalid subs type', { main, subs });
          }
        } catch (mainError) {
    ...
    switch (cleanCommand) {
      case 'search': {
        ...
        searchResults.forEach((template, index) => {
          try {
            if (!template || typeof template !== 'object') {
              logWarning('main.search.display', 'Invalid template object', { templateIndex: index });
              return;
            }
      ...
      case 'trigger': {
        ...
        if (!trigger) {
          const error = new Error('请提供触发词');
          error.code = 'MISSING_TRIGGER';
          error.operation = 'main.trigger';
          throw error;
        }
        ...
        triggerResults.forEach((template, index) => {
          try {
            if (!template || typeof template !== 'object') {
              logWarning('main.trigger.display', 'Invalid template object', { templateIndex: index });
              return;
            }
      ...
      case 'show': {
        ...
        if (!templateId) {
          const error = new Error('请提供模板ID');
          error.code = 'MISSING_TEMPLATE_ID';
          error.operation = 'main.show';
          throw error;
        }
      ...
      case 'recommend': {
        ...
        recommendations.forEach((template, index) => {
          try {
            if (!template || typeof template !== 'object') {
              logWarning('main.recommend.display', 'Invalid template object', { templateIndex: index });
              return;
            }
      ...
      default:
        const error = new Error(`未知命令: ${cleanCommand}`);
        error.code = 'UNKNOWN_COMMAND';
        error.operation = 'main';
        error.details = { command: cleanCommand };
        throw error;
    }
