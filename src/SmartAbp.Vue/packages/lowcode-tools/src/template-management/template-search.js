...
class TemplateSearcher {
      ...
      const results = this.index.templates.filter((template) => {
        try {
          ...
          if (searchOptions.tags && searchOptions.tags.length > 0) {
            ...
            const hasTag = searchOptions.tags.some((tag) => {
              ...
              return template.tags.some((templateTag) => {
                ...
                const templateTagText = searchOptions.caseSensitive
                  ? templateTag
          ...
          if (query && query.trim() !== "") {
            ...
            // Search in name
            if (template.name) {
              const nameText = searchOptions.caseSensitive
                ? template.name
      ...
      const results = this.index.templates
        .filter((template) => {
          try {
            ...
            logWarning(
              "searchByTrigger.filter",
              "Failed to process template during trigger search",
              {
                templateId: template.id,
                error: templateError.message,
              },
        ...
        candidates = candidates.filter((t) => {
          try {
            ...
            logWarning(
              ...
              "Failed to filter template by entity type",
              {
                templateId: t.id,
          ...
          try {
            ...
            const hasTrigger =
              ...
              t.ai_triggers.some((trigger) => {
                validateString(trigger, "AI trigger")
                const triggerText = this.options.caseSensitive ? trigger : trigger.toLowerCase()
                const operationText = this.options.caseSensitive
                  ? operation
          ...
          try {
            ...
            const hasDependency =
              ...
              t.dependencies.some((dep) => {
                ...
                const frameworkText = this.options.caseSensitive
                  ? framework
  ...
  showTemplate(templateId) {
    try {
      ...
      if (
        template.ai_triggers &&
        Array.isArray(template.ai_triggers) &&
        template.ai_triggers.length > 0
      ) {
      ...
      if (
        template.scenarios &&
        Array.isArray(template.scenarios) &&
      ...
      if (
        ...
        template.parameters.forEach((param, index) => {
          try {
            ...
            console.log(
              `  ${index + 1}. ${param.name || "Unknown"} (${param.type || "unknown"}): ${param.description || "No description"}`,
            )
            if (param.example) {
  ...
  getStats() {
    const avgResultsCount =
      this.stats.searchesPerformed > 0
