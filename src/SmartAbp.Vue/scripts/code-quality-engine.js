...
class CodeQualityEngine {
  ...
  async checkCodeStyle() {
    ...
    try {
      ...
      console.log(chalk.green(`✅ 代码风格检查完成 - 得分: ${this.qualityMetrics.codeStyle.score}/100`))
    } catch (error) {
  ...
  async checkSecurity() {
    ...
    try {
      ...
      this.qualityMetrics.security.score = securityIssues.length === 0 ? 100 : 80 - (securityIssues.length * 10)
  ...
  async checkPerformancePatterns() {
    ...
    try {
      ...
      this.qualityMetrics.performance.score = performanceIssues.length === 0 ? 100 : 85 - (performanceIssues.length * 5)
      
      console.log(chalk.green(`✅ 性能检查完成 - 得分: ${this.qualityMetrics.performance.score}/100`))
    } catch (error) {
  ...
  async checkMaintainability() {
    ...
    try {
      ...
      this.qualityMetrics.maintainability.score = maintainabilityIssues.length === 0 ? 100 : 90 - (maintainabilityIssues.length * 2)
      
      console.log(chalk.green(`✅ 可维护性检查完成 - 得分: ${this.qualityMetrics.maintainability.score}/100`))
    } catch (error) {
  ...
  async checkTestCoverage() {
    ...
    try {
      ...
      this.qualityMetrics.testCoverage.issues = coverage.belowThreshold ? ['测试覆盖率低于阈值'] : []
      
      console.log(chalk.green(`✅ 测试覆盖率检查完成 - 得分: ${this.qualityMetrics.testCoverage.score}/100`))
    } catch (error) {
  ...
  async validateGeneratedCode() {
    ...
    try {
      ...
      console.log(chalk.green(`✅ 生成代码验证完成 - 文件: ${generatedFiles.length}, 行数: ${totalLines}`))
    } catch (error) {
