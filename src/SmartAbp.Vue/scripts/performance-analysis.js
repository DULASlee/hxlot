...
class AdvancedPerformanceAnalyzer {
  ...
  async analyzeBuildPerformance() {
    ...
    return {
      ...
      formatted: {
        average: this.formatDuration(buildTimes.reduce((sum, time) => sum + time, 0) / buildTimes.length),
  ...
  async analyzeTreeShaking() {
    ...
    try {
      ...
      return {
        ...
        effectiveness: savingsPercentage > 20 ? 'Excellent' : 
                      savingsPercentage > 10 ? 'Good' : 
                      savingsPercentage > 5 ? 'Fair' : 'Poor'
      }
    } catch (error) {
      return { error: error.message }
    }
  ...
  generateRecommendations(reports) {
    ...
    if (reports.bundleReport.totalSize?.raw > 2000000) { // 2MB
      recommendations.push({
        type: 'Bundle Size',
        priority: 'High',
        issue: 'Large bundle size detected',
        recommendation: 'Consider implementing dynamic imports and lazy loading for non-critical components',
    ...
    if (reports.lighthouseReport.performance < 90) {
      recommendations.push({
        ...
        recommendation: 'Optimize images, implement service workers, and reduce JavaScript execution time',
  ...
  displaySummary(report) {
    ...
    if (report.lighthouse.performance) {
      const color = report.lighthouse.performance >= 90 ? 'green' : 
                   report.lighthouse.performance >= 70 ? 'yellow' : 'red'
      console.log(`Lighthouse Score: ${chalk[color](report.lighthouse.performance.toFixed(0) + '/100')}`)
    }
  ...
  calculateSplittingEffectiveness(chunks) {
    ...
    return {
      ...
      rating: effectiveness > 0.7 ? 'Excellent' :
              effectiveness > 0.5 ? 'Good' :
              effectiveness > 0.3 ? 'Fair' : 'Poor'
    }
  }
}
