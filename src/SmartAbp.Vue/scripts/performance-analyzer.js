...
class PerformanceAnalyzer {
  ...
  async analyzeCodePerformance() {
    ...
    console.log(
      chalk.green(
        `✅ 代码性能分析完成 - 发现 ${this.getIssuesByType("CODE_PERFORMANCE").length} 个问题`,
      ),
    )
  }
  ...
  async analyzeDatabasePerformance() {
    ...
    console.log(
      chalk.green(
        `✅ 数据库性能分析完成 - 发现 ${this.getIssuesByType("DATABASE_PERFORMANCE").length} 个问题`,
      ),
    )
  }
  ...
  async analyzeMemoryUsage() {
    ...
    console.log(
      chalk.green(
        `✅ 内存使用分析完成 - 发现 ${this.getIssuesByType("MEMORY_USAGE").length} 个问题`,
      ),
    )
  }
  ...
  getPerformanceSeverity(pattern) {
    const criticalPatterns = [/\.Result/, /\.Wait\(\)/, /TransactionScope.*new/]
