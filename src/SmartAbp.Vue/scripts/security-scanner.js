...
class SecurityScanner {
  ...
  async scanForSqlInjection() {
    ...
    console.log(chalk.green(`✅ SQL注入扫描完成 - 发现 ${this.getIssuesByType('SQL_INJECTION').length} 个问题`))
  }
  ...
  async scanForHardcodedSecrets() {
    ...
    console.log(chalk.green(`✅ 敏感信息扫描完成 - 发现 ${this.getIssuesByType('HARDCODED_SECRET').length} 个问题`))
  }
  ...
  async scanForXssVulnerabilities() {
    ...
    console.log(chalk.green(`✅ XSS漏洞扫描完成 - 发现 ${this.getIssuesByType('XSS_VULNERABILITY').length} 个问题`))
  }
  ...
  async scanForInsecureDeserialization() {
    ...
    console.log(chalk.green(`✅ 反序列化扫描完成 - 发现 ${this.getIssuesByType('INSECURE_DESERIALIZATION').length} 个问题`))
  }
  ...
  async scanForAuthenticationIssues() {
    ...
    console.log(chalk.green(`✅ 认证授权扫描完成 - 发现 ${this.getIssuesByType('AUTHENTICATION_ISSUE').length} 个问题`))
  }
  ...
  scanDirectory(dir, callback) {
    ...
    items.forEach(item => {
      ...
      if (stat.isDirectory()) {
        ...
        // 检查文件模式
        const shouldScan = this.scanConfig.filePatterns.some(pattern => 
          pattern.test(item)
        )
  ...
  generateSecurityReport() {
    ...
    return {
      ...
      status: criticalIssues.length > 0 ? 'CRITICAL' : 
              ...
              this.securityIssues.length > 0 ? 'MEDIUM' : 'PASSED'
    }
  }
