    ...
    for (const line of lines) {
      if (line.includes('Tests') && line.includes('passed') && line.includes('failed')) {
        ...
        if (match) {
          ...
          this.testResults[currentSuite].total = this.testResults[currentSuite].passed + this.testResults[currentSuite].failed
        }
      }
  ...
  generateReport() {
    const report = {
      ...
      summary: {
        ...
        successRate: ((Object.values(this.testResults).reduce((sum, suite) => sum + suite.passed, 0) / 
                      Object.values(this.testResults).reduce((sum, suite) => sum + suite.total, 0)) * 100).toFixed(2)
      },
