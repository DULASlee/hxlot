...
export class DragDropEngine {
  ...
  private handleMouseMove(event: MouseEvent): void {
    ...
    // 网格对齐
    if (this.config.enableSnap) {
      currentPosition.x = Math.round(currentPosition.x / this.config.gridSize) * this.config.gridSize
      currentPosition.y = Math.round(currentPosition.y / this.config.gridSize) * this.config.gridSize
    }
    ...
    if (nodeId) {
      ...
      this.callbacks.onDragEnd.forEach(callback => {
        callback({
          ...
          moved: this.dragState.startPosition.x !== this.dragState.currentPosition.x ||
  ...
  public getSelectedComponents(): DesignerComponent[] {
    return Array.from(this.selectionState.selectedIds)
      ...
      .filter((component): component is DesignerComponent =>
