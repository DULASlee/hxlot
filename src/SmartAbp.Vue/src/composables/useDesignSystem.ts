...
export function useDesignSystem(): UseDesignSystemReturn {
  ...
  const theme: Ref<string> = ref(localStorage.getItem(THEME_STORAGE_KEY) || 'tech-blue')
  const isDarkMode: Ref<boolean> = ref(
    localStorage.getItem(DARK_MODE_STORAGE_KEY) === 'true'
  )

  const isCurrentThemeDark: ComputedRef<boolean> = computed(() => {
  ...
  const preloadTheme = (themeType: string): void => {
  ...
  const performanceMonitor = {
    startTime: 0,
    endTime: 0,
    measureThemeSwitch: (themeName: string): void => {
      performanceMonitor.startTime = performance.now()
      console.log(`🎨 开始切换主题: ${themeName}`)
    },
    endMeasurement: (themeName: string): void => {
