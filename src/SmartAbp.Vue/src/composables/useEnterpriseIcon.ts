...
import { iconManager, type IconConfig, IconCategory } from '@/components/icons/IconManager'
 ...
 */
export function useEnterpriseIcon(options: UseIconOptions) {
  // 📊 响应式状态
  const isLoading = ref(false)
  const isLoaded = ref(false)
  const error = ref<Error | null>(null)
  const iconConfig = ref<IconConfig | null>(null)
  ...
  const iconComponent = computed(() => {
    if (!iconConfig.value) return null
  ...
  const loadIcon = async () => {
    ...
    try {
      ...
      if (options.preload && typeof config.component === 'function') {
        const asyncComponent = config.component as () => Promise<any>
...
export function useIconSearch() {
  ...
  const searchResults = ref<IconConfig[]>([])
  ...
  // 🔍 搜索图标
  const searchIcons = async (options: IconSearchOptions) => {
...
export function useIconTheme() {
  ...
  // 🎨 更新主题
  const updateTheme = async (theme: Partial<typeof currentTheme.value>) => {
...
export function useIconStats() {
  // 📊 响应式状态
  const stats = ref<IconUsageStats>({
    totalIcons: 0,
    loadedIcons: 0,
    categoryStats: {},
    mostUsedIcons: []
