...
import type { MenuItem, PageMenuItem } from '@/types/menu'
...
export function useMenuPermission() {
   ...
   * 获取扁平的所有可访问菜单
   */
  const flatAccessibleMenus = computed<MenuItem[]>(() => {
    const roles = authStore.userInfo?.roles || []
  ...
  const hasPathPermission = (path: string): boolean => {
    ...
    if (!menu) return true // 没有配置菜单的路径默认允许访问
  ...
  const accessiblePaths = computed<string[]>(() => {
    return flatAccessibleMenus.value
      .filter((menu): menu is PageMenuItem => menu.type === 'page')
      .map(menu => menu.path)
  })
   ...
   * 搜索可访问的菜单
   */
  const searchAccessibleMenus = (keyword: string): MenuItem[] => {
    const roles = authStore.userInfo?.roles || []
