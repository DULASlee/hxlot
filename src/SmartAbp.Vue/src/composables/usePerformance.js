...
export function usePerformance() {
    ...
    const getPerformanceScore = () => {
        ...
        if (m.firstContentfulPaint) {
            if (m.firstContentfulPaint > 3000)
                score -= 20;
            else if (m.firstContentfulPaint > 1800)
                score -= 10;
        }
        // LCP评分 (< 2.5s = 100, > 4s = 0)
        if (m.largestContentfulPaint) {
            if (m.largestContentfulPaint > 4000)
                score -= 20;
            else if (m.largestContentfulPaint > 2500)
                score -= 10;
        }
        // FID评分 (< 100ms = 100, > 300ms = 0)
        if (m.firstInputDelay) {
            if (m.firstInputDelay > 300)
                score -= 20;
            else if (m.firstInputDelay > 100)
                score -= 10;
        }
        // CLS评分 (< 0.1 = 100, > 0.25 = 0)
        if (m.cumulativeLayoutShift) {
            if (m.cumulativeLayoutShift > 0.25)
                score -= 20;
            else if (m.cumulativeLayoutShift > 0.1)
                score -= 10;
        }
