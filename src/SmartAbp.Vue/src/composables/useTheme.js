import { flattenPalette, generateThemePalettes } from "@/styles/tokens/colorPalette";
...
const currentTheme = ref(localStorage.getItem("app-theme") || THEMES.LIGHT);
        ...
        requestAnimationFrame(() => {
            ...
            if (config?.palettes) {
                Object.entries(config.palettes).forEach(([paletteName, palette]) => {
                    const flatVars = flattenPalette(paletteName, palette);
...
export const themeUtils = {
    getThemeColor: (colorKey, themeName = null) => {
        const target = themeName || currentTheme.value;
        return themeConfig[target]?.colors[colorKey];
    },
    generateGradient: (color1, color2, direction = "to right") => {
