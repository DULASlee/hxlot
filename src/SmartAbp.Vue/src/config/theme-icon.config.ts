 ...
 * ```
 */
export function isValidIconStyle(style: string): boolean {
  return Object.values(THEME_ICON_BINDING).includes(style as IconStyleType)
}
...
export function getAvailableThemes(): ThemeType[] {
  return Object.keys(THEME_ICON_BINDING) as ThemeType[]
}
 ...
 * @returns 图标风格类型数组（去重）
 */
export function getUsedIconStyles(): IconStyleType[] {
