...
async function runAdvancedExample() {
  ...
  const schemas: Vue3Schema[] = [
    ...
    },
    {
      ...
      props: [{ name: "modelValue", type: "string", required: true }],
      emits: [{ name: "update:modelValue", payload: "string" }],
    },
  ]
  ...
  console.log(
    `批量生成完成，成功：${batchResults.filter((r) => r.success).length}/${batchResults.length}`,
  )
...
// 导出示例函数
export { basicExample, runAdvancedExample, performanceTest }
