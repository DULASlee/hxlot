...
export class CacheManager implements ICacheManager {
  ...
  getMemoryUsage(): CacheMemoryInfo {
    const used = Array.from(this.cache.values()).reduce((total, entry) => total + entry.size, 0)
  ...
  private evictByMemory(requiredSpace: number): void {
    ...
    if (freedSpace < requiredSpace) {
      ...
      const candidates = this.strategy.getEvictionCandidates(this.cache, targetEvictions)
