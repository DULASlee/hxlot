...
export class EventBus implements IEventBus {
  ...
  private eventQueue = new Map<
    ...
    Array<{ data: any; resolve: () => void; reject: (err: Error) => void }>
  >()
  ...
  off<K extends keyof LowCodeEventMap>(event: K, handler: EventHandler<LowCodeEventMap[K]>): void {
  ...
  async emit<K extends keyof LowCodeEventMap>(event: K, data: LowCodeEventMap[K]): Promise<void> {
    // 如果正在处理此事件，加入队列等待
  ...
  private async processEventQueue<K extends keyof LowCodeEventMap>(event: K): Promise<void> {
  ...
  emitSync<K extends keyof LowCodeEventMap>(event: K, data: LowCodeEventMap[K]): void {
    this.recordEvent(event, data)
  ...
  getEventHistory<K extends keyof LowCodeEventMap>(
    ...
    for (const [eventName, events] of this.eventHistory) {
      allHistory.push(
        ...events.map((data) => ({ event: eventName, data, timestamp: data.timestamp })),
      )
    }

    return allHistory.sort((a, b) => b.timestamp - a.timestamp).slice(0, limit || 100)
  }
  ...
  private recordEvent<K extends keyof LowCodeEventMap>(event: K, data: LowCodeEventMap[K]): void {
