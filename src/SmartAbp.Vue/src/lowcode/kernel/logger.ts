...
import type { StructuredLogger as IStructuredLogger, LogLevel, LogEntry } from "./types"
...
export class StructuredLogger implements IStructuredLogger {
  ...
  private flushBatch(): void {
    ...
    // 异步写入批量日志
    Promise.allSettled(entriesToFlush.map((entry) => this.writeEntry(entry))).catch((error) => {
