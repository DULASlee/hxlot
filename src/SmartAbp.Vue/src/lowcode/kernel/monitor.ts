...
export class MetricStorage {
  ...
  aggregate(name: string, timeRange: { start: number; end: number }): AggregatedMetrics | null {
  ...
  private cleanup(): void {
    ...
    this.metrics = this.metrics.filter((entry) => entry.metric.timestamp > cutoff)
  }
...
export class PerformanceMonitor implements IPerformanceMonitor {
  ...
  private calculateHealthScore(metrics: PerformanceMetric[]): number {
    ...
    const slowOperations = metrics.filter(
      (m) => m.name.includes("duration") && m.value > 1000, // > 1秒
    )
  ...
  getHealthMetrics(): Record<string, any> {
    ...
    const generationMetrics = recentMetrics.filter((m) => m.name.startsWith("generation."))
