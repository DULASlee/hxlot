...
export class RouterGeneratorPlugin
  implements CodegenPlugin<Schema, RouterGeneratorConfig, GeneratedCode>
{
  ...
  canHandle(schema: Schema): boolean {
    return schema.type === "page" || schema.type === "layout"
  }
  ...
  async generate(
    ...
    try {
      ...
      const code = this.emitRouterCode(schema, {
        base,
        mode,
        lazy,
        guards,
  ...
  private emitRouterCode(
    ...
    const componentPath =
      schema.metadata?.componentPath ||
      "@/views/" + (schema.metadata?.module || "common") + "/" + routeName + ".vue"
