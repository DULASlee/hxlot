...
export class SfcCompilerEngine {
  ...
  private extractComponentName(descriptor: SFCDescriptor): string {
    ...
    if (descriptor.scriptSetup?.content) {
      const nameMatch = descriptor.scriptSetup.content.match(/defineOptions\s*\(\s*{\s*name\s*:\s*['"]([^'"]+)['"]/);
      if (nameMatch) return nameMatch[1];
    }
  ...
  private extractExports(descriptor: SFCDescriptor, compiledScript: string): string[] {
    ...
    if (exportMatches) {
      exportMatches.forEach(exportStatement => {
        const nameMatch = exportStatement.match(/export\s+(?:const|let|var|function|class)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/);
        if (nameMatch) {
