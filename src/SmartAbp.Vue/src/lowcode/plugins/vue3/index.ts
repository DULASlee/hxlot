...
import { SfcCompilerPlugin, type CompiledSFC, type SfcCompilerConfig, type PreviewContext } from '../sfc-compiler/index';
...
${spaces}</${node.tag}>`;
  ...
  private generateMethod(method: MethodDefinition): string {
    ...
    const returnType = this.config.typescript && method.returnType
      ? `: ${method.returnType}`
      : '';
...
export class Vue3Plugin implements CodegenPlugin<Vue3Schema, Vue3Config, Vue3GeneratedCode> {
    ...
    try {
      ...
      if (mergedConfig.enableCompilation && this.compiler) {
        ...
        try {
          ...
          context.logger.warn('SFC编译失败，继续返回源码', { error: (compileError as Error).message });
          // 编译失败不阻断生成流程，仅返回源码
        }
      }
   ...
   */
  async createPreview(
    generatedCode: Vue3GeneratedCode,
