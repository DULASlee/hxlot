...
export class MetadataPipeline {
  ...
  async processMetadata(input: RawMetadata): Promise<UnifiedMetadata> {
    ...
    try {
      ...
      const promise = (async () => {
        ...
        if (useWorkers) {
          if (!this.pool) {
            ...
            try {
              ;(globalThis as any).__metadataWorkerPool = this.pool
            } catch {}
          }
        ...
        try {
          ...
          globalContentCache.set(key, bytes, {
            type: "metadata",
            size: bytes.byteLength,
            tags: ["metadata", MODEL_VERSION],
          })
        } catch {}
      ...
      const duration = (performance.now?.() ?? Date.now()) - start
      await this.context.eventBus.emit("metadata:recovered", {
        duration,
        strategy: "minimal",
        timestamp: Date.now(),
      })
  ...
  private async emitProcessed(duration: number, cacheHit: boolean): Promise<void> {
    ...
    await this.context.eventBus.emit("metadata:processed", {
      key: "metadata",
      duration,
      cacheHit,
      timestamp: Date.now(),
    ...
    try {
      if (typeof dispatchEvent !== "undefined") {
        dispatchEvent(
          new CustomEvent("lowcode:metadata:processed", { detail: { duration, cacheHit } }),
        )
      }
    } catch {}
  }
  ...
  private async recoverFromError(_error: Error, input: RawMetadata): Promise<UnifiedMetadata> {
    // 最小降级：仅保留基本可用字段
    const minimal = this.transformToUnified({
