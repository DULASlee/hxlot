...
class TemplateSearcher {
  ...
  search(query, options = {}) {
    ...
    const results = this.index.templates.filter(template => {
      ...
      if (options.tags && options.tags.length > 0) {
        const hasTag = options.tags.some(tag => template.tags.some(t => t.toLowerCase().includes(tag.toLowerCase())));
  ...
  searchByTrigger(trigger) {
    ...
    return this.index.templates.filter(template => template.ai_triggers.some(t => t.toLowerCase().includes(triggerLower)))
      .sort((a, b) => this.calculateMatchScore(b, triggerLower) - this.calculateMatchScore(a, triggerLower));
  }

  calculateMatchScore(template, trigger) {
    let score = 0;
    template.ai_triggers.forEach(t => {
      if (t.toLowerCase() === trigger) score += 10; else if (t.toLowerCase().includes(trigger)) score += 5;
    });
    template.tags.forEach(tag => { if (tag.toLowerCase().includes(trigger)) score += 3; });
  ...
  getRecommendations(context = {}) {
    ...
    if (entityType) candidates = candidates.filter(t => t.scenarios.some(s => s.includes(entityType)) || t.ai_triggers.some(trigger => trigger.includes(entityType)));
    if (operation) candidates = candidates.filter(t => t.ai_triggers.some(trigger => trigger.includes(operation)) || t.tags.some(tag => tag.includes(operation)));
    if (framework) candidates = candidates.filter(t => t.dependencies.some(dep => dep.toLowerCase().includes(framework.toLowerCase())) || t.category.includes(framework.toLowerCase()));
    return candidates.sort((a, b) => (b.dependencies.length + b.ai_triggers.length) - (a.dependencies.length + a.ai_triggers.length)).slice(0, 5);
  }

  showTemplate(templateId) {
    if (!this.index) throw new Error('模板索引未初始化');
    const template = this.index.templates.find(t => t.id === templateId);
    if (!template) { console.log(`❌ 未找到模板: ${templateId}`); return; }
    console.log(`\n📋 模板详情: ${template.name}`);
    ...
    if (template.ai_triggers.length > 0) console.log(`🎯 AI触发词: ${template.ai_triggers.join(', ')}`);
    if (template.scenarios.length > 0) console.log(`💡 适用场景: ${template.scenarios.join(', ')}`);
    if (template.dependencies.length > 0) console.log(`📦 依赖项: ${template.dependencies.join(', ')}`);
...
async function main() {
  ...
  switch (command) {
    ...
    case 'trigger': {
      ...
      if (!trigger) { console.error('❌ 请提供触发词'); process.exit(1); }
    ...
    case 'show': {
      ...
      if (!templateId) { console.error('❌ 请提供模板ID'); process.exit(1); }
