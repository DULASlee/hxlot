...
export class AdvancedComponentLoader {
    constructor() {
        Object.defineProperty(this, "loadingComponents", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "loadedComponents", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    async loadComponentWithMetrics(componentName, importFunction) {
        // Check cache first
        ...
        const loadingPromise = importFunction()
            ...
            performance.measure(`component-load-${componentName}`, `component-load-start-${componentName}`, `component-load-end-${componentName}`);
            // Store metrics
...
export const AdvancedComponents = {
    ...
    AdvancedTable: componentLoader.createLazyComponent("AdvancedTable", () => import("@/packages/lowcode-designer/src/components/AdvancedTable/AdvancedTable.vue"), { delay: 0 }),
    AdvancedForm: componentLoader.createLazyComponent("AdvancedForm", () => import("@/packages/lowcode-designer/src/components/AdvancedForm/AdvancedForm.vue"), { delay: 0 }),
    // Visualization components - lazy load
    AdvancedChart: componentLoader.createLazyComponent("AdvancedChart", () => import("@/packages/lowcode-designer/src/components/AdvancedChart/AdvancedChart.vue"), { delay: 100 }),
    // Layout components - progressive load
    AdvancedLayout: componentLoader.createLazyComponent("AdvancedLayout", () => import("@/packages/lowcode-designer/src/components/AdvancedLayout/AdvancedLayout.vue"), { delay: 200 }),
    AdvancedNavigation: componentLoader.createLazyComponent("AdvancedNavigation", () => import("@/packages/lowcode-designer/src/components/AdvancedNavigation/AdvancedNavigation.vue"), { delay: 200 }),
    AdvancedPanel: componentLoader.createLazyComponent("AdvancedPanel", () => import("@/packages/lowcode-designer/src/components/AdvancedPanel/AdvancedPanel.vue"), { delay: 200 }),
    // Heavy components - lazy load with longer delay
    AdvancedTree: componentLoader.createLazyComponent("AdvancedTree", () => import("@/packages/lowcode-designer/src/components/AdvancedTree/AdvancedTree.vue"), { delay: 300, timeout: 15000 }),
    AdvancedUpload: componentLoader.createLazyComponent("AdvancedUpload", () => import("@/packages/lowcode-designer/src/components/AdvancedUpload/AdvancedUpload.vue"), { delay: 300, timeout: 15000 }),
};
...
export const PerformanceMonitor = {
    ...
    onVitalMetric(metric) {
        const threshold = performanceConfig.vitals[metric.name];
        const isGood = metric.value <= threshold;
        console.log(`%c${metric.name}: ${metric.value.toFixed(2)}${metric.name === "CLS" ? "" : "ms"} ${isGood ? "✅" : "❌"}`, `color: ${isGood ? "green" : "red"}; font-weight: bold;`);
    ...
    measureComponentRender(componentName, renderFn) {
        ...
        requestAnimationFrame(() => {
            ...
            performance.measure(`render-${componentName}`, `render-start-${componentName}`, `render-end-${componentName}`);
            if (renderTime > 16) {
                // More than one frame
    ...
    getPerformanceEntries() {
        ...
        const componentEntries = entries.filter((entry) => entry.name.startsWith("component-load-") || entry.name.startsWith("render-"));
...
export const BundleOptimizer = {
    ...
    checkDuplicates() {
        ...
        const actualDuplicates = Array.from(duplicates.entries()).filter(([_, instances]) => instances.length > 1);
