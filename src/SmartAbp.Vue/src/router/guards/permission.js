 ...
 * 权限守卫 - 检查用户是否有权限访问路由
 */
export const permissionGuard = async (to) => {
    ...
    if (isSuperUserAccount) {
        logger.debug(`[权限守卫] 超级用户 "${username}" 白名单放行（开发阶段） - 拥有所有权限`);
        return true;
    }
    ...
    if (requiredRoles && requiredRoles.length > 0) {
        ...
        if (!hasPermission) {
            ...
            logger.warn(`[权限守卫] 用户权限不足 - 用户: ${username}, 需要角色: ${requiredRoles.join(', ')}, 用户最高角色: ${highestRole}, 所有角色: ${userRoles.join(', ')}`);
            ElMessage.warning({
...
export const roleGuard = (requiredRoles) => {
    return async (_to) => {
        ...
        if (!hasPermission) {
            const highestRole = getHighestRole(userRoles);
            logger.warn(`[角色守卫] 权限不足 - 需要: ${requiredRoles.join(', ')}, 用户最高角色: ${highestRole}`);
            return { name: 'Forbidden' };
        }
        return true;
    };
 ...
 */
export const adminGuard = async (_to) => {
    const authStore = useAuthStore();
    const userRoles = authStore.userInfo?.roles || [];
    // 🏛️ 使用角色层级系统检查管理员权限
    ...
    if (!isAdmin(userRoles)) {
        const highestRole = getHighestRole(userRoles);
        logger.warn(`[管理员守卫] 非管理员尝试访问管理页面 - 用户最高角色: ${highestRole}`);
        ElMessage.warning({
            message: '仅管理员可访问此页面',
            duration: 3000,
            showClose: true
