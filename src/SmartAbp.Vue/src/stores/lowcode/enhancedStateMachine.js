...
export const useEnhancedStateMachineStore = defineStore("enhancedStateMachine", () => {
    ...
    const removeState = (stateId) => {
        // 移除相关转换
        transitions.value = transitions.value.filter(t => t.source !== stateId && t.target !== stateId);
        // 移除状态
    ...
    const addTransition = (transition) => {
        ...
        // 检查重复转换
        const existingTransition = transitions.value.find(t => t.source === transition.source && t.target === transition.target);
        ...
        try {
            for (const rule of rulesToExecute) {
                ...
                try {
                    ...
                    logger.error(`规则执行错误 ${rule.id}:`, error);
                }
            }
        }
        finally {
    ...
    const executeRuleAction = async (rule, context) => {
        // 实现具体的动作执行逻辑
        ...
        try {
            ...
            logger.error(`规则动作执行失败: ${rule.action}`, error);
            throw error; // 重新抛出错误，让上层处理
        }
    };
    const logRuleExecution = (ruleId, executionInfo) => {
    ...
    // === 代码生成功能 ===
    const generateFrontendHooks = (workflowName) => {
...
async function executeBusinessRules(trigger: string, context: Record<string, any>) {
  ...
  return context
    ...
    const generateBackendHandlers = (workflowName) => {
        return `// AUTO-GENERATED FILE - DO NOT EDIT
using System;
...
${transitions.value.map(t => `
    ...
    const generatePolicies = (workflowName) => {
        const permissionRules = businessRules.value.filter(r => r.type === "permission-constraint");
        return `// AUTO-GENERATED FILE - DO NOT EDIT
using System.Threading.Tasks;
...
${permissionRules.map(rule => `
    ...
    const generateCompleteCodePackage = (options = {
        frontend: true,
        backend: true,
        policies: true,
        tests: true
    ...
    const generateTestCode = (workflowName) => {
        return `// AUTO-GENERATED FILE - DO NOT EDIT
...
${transitions.value.map(t => `
  it("should execute ${t.source} to ${t.target} transition", async () => {
    ...
    // === 工作流模板管理 ===
    const findWorkflowTemplates = (keyword) => {
        return workflowTemplates.value.filter(template => template.name.toLowerCase().includes(keyword.toLowerCase()) ||
            template.description.toLowerCase().includes(keyword.toLowerCase()));
    };
    const addWorkflowTemplate = (template) => {
    ...
    // === 元数据管理 ===
    const setWorkflowMetadata = (metadata) => {
