...
// 🔄 合并扩展映射到主映射表
Object.assign(ICON_MAPPINGS, ICON_MAPPINGS_EXTEND_DOCS, ICON_MAPPINGS_EXTEND_TEST, ICON_MAPPINGS_EXTEND_AUTH, ICON_MAPPINGS_EXTEND_AI, ICON_MAPPINGS_EXTEND_INTERACTION, ICON_MAPPINGS_EXTEND_THEME);
// 🏪 图标风格管理 Store
export const useIconStyleStore = defineStore('iconStyle', () => {
    // 📊 状态
    // ✅ 配置驱动：使用配置中心的默认值（Element Plus）
    const currentStyle = ref(DEFAULT_VALUES.ICON_STYLE);
    ...
    const availableStyles = computed(() => Object.values(ICON_STYLES).filter(style => style.enterprise));
    /**
     * 🔍 从旧的图标类名中提取图标键名
     * @param iconClass 旧的图标类名（如 'fas fa-chart-pie'）
     * @returns 图标键名（如 'chart-pie'） 或 null
     */
    const extractIconKey = (iconClass) => {
    ...
    const getIcon = (key) => {
        ...
        const styleKey = currentStyle.value === 'element-plus' ? 'elementPlus' : currentStyle.value;
     ...
     * @param style 目标图标风格
     */
    const setIconStyle = async (style) => {
     ...
     * @param mapping 图标映射
     */
    const addIconMapping = (key, mapping) => {
