...
export const useMenuStore = defineStore('menu', () => {
   ...
   * 扁平化的菜单映射（方便查找）
   */
  const menuMap = computed<Map<string, MenuItem>>(() => {
    const map = new Map<string, MenuItem>()
    
    const collectMenus = (menus: MenuItem[]): void => {
      menus.forEach(menu => {
   ...
   * 当前激活子菜单的信息
   */
  const activeSubMenu = computed<MenuItem | null>(() => {
   ...
   * 副菜单标题
   */
  const submenuTitle = computed<string>(() => {
   ...
   * 当前激活菜单的子菜单列表
   */
  const currentSubmenuItems = computed<MenuItem[]>(() => {
   ...
   * 是否应该显示副菜单
   */
  const shouldShowSubmenu = computed<boolean>(() => {
    return (
   ...
   * 权限摘要信息（便于调试）
   */
  const permissionSummary = computed<PermissionSummary>(() => {
   ...
   * 设置默认菜单状态
   */
  const setupDefaultMenuState = (): void => {
   ...
   * 切换菜单展开状态
   */
  const toggleMenuExpansion = (menuKey: string): void => {
   ...
   * 设置激活的菜单
   */
  const setActiveMenu = (menuKey: string, subMenuKey?: string): void => {
    menuState.value.activeMenuKey = menuKey
   ...
   * 添加标签页
   */
  const addTab = (page: MenuItem): void => {
    if (page.type !== 'page') {
      return
    }
   ...
   * 切换标签页
   */
  const switchTab = (tabKey: string): string | null => {
   ...
   * 关闭标签页
   */
  const closeTab = (tabKey: string): string | null => {
   ...
   * 根据路径更新菜单状态
   */
  const updateMenuStateByPath = (path: string): boolean => {
   ...
   * 切换单标签模式
   */
  const toggleSingleTabMode = (): void => {
