...
class InputValidator {
     ...
     * 验证TypeScript文件
     */
    validateTypeScriptFile(filePath, content) {
        const lines = content.split("\n");
        lines.forEach((line, lineIndex) => {
            this.checkSystemName(filePath, line, lineIndex + 1);
            this.checkRouteName(filePath, line, lineIndex + 1);
     ...
     * 验证JavaScript文件
     */
    validateJavaScriptFile(filePath, content) {
        const lines = content.split("\n");
        lines.forEach((line, lineIndex) => {
            this.checkInputValidation(filePath, line, lineIndex + 1);
     ...
     * 检查系统名称格式
     */
    checkSystemName(filePath, line, lineNumber) {
        // 查找系统名称定义
     ...
     * 检查路由名称格式
     */
    checkRouteName(filePath, line, lineNumber) {
        const routePatterns = [/path\s*:\s*["']([^"']+)["']/gi, /route\s*:\s*["']([^"']+)["']/gi];
     ...
     * 检查权限名称格式
     */
    checkPermissionName(filePath, line, lineNumber) {
     ...
     * 检查输入验证
     */
    checkInputValidation(filePath, line, lineNumber) {
        // 检查是否有直接的用户输入处理
        const inputPatterns = [
            /req\.body/gi,
            /req\.query/gi,
     ...
     * 检查Zod模式定义
     */
    checkZodSchemas(filePath, line, lineNumber) {
        // 检查Zod模式定义是否正确
        const zodPatterns = [
            /z\.string\(\)\.min\(/gi,
            /z\.string\(\)\.max\(/gi,
     ...
     * 检查危险输入
     */
    checkDangerousInputs(filePath, line, lineNumber) {
