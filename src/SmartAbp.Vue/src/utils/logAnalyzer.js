...
class LogAnalyzer {
    ...
    // 分析性能
    analyzePerformance(performanceEntries) {
    ...
    // 生成洞察
    generateInsights(logs, performanceEntries) {
...
<body>
    ...
    generateCsvReport(analysis) {
        ...
        const rows = [
            ...
            [
                "慢操作数",
                analysis.performance.slowOperations.length.toString(),
                "响应时间超过1秒的操作数",
            ],
        ];
...
export const setAnalysisConfig = (config) => logAnalyzer.setConfig(config);
export const exportAnalysisReport = (format = "json") => logAnalyzer.exportReport(format);
