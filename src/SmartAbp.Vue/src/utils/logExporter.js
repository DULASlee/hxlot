...
class LogExporter {
    ...
    // 过滤日志
    filterLogs(logs, config) {
        let filteredLogs = [...logs];
        // 时间范围过滤
        if (config.timeRange) {
            filteredLogs = filteredLogs.filter((log) => log.timestamp >= config.timeRange.start.getTime() &&
        ...
        if (config.categories && config.categories.length > 0) {
            filteredLogs = filteredLogs.filter((log) => log.category && config.categories.includes(log.category));
        }
        // 限制条目数量
...
// 导出便捷方法
export const exportLogs = (config) => logExporter.exportLogs(config);
export const downloadLogs = (config, filename) => logExporter.downloadLogs(config, filename);
export const previewExport = (config) => logExporter.previewExport(config);
// 预设配置
