 ...
 */
import { logger, lowcodeLogger, createAppLogger, createComponentLogger, createServiceLogger, createPluginLogger, log, perf, system, userActivity, api, database, useLogger, usePerformanceTracking, ConsoleTransport, FileTransport, MemoryTransport, NetworkTransport, LogLevel, } from "./index";
/**
 * 示例1: 基础日志使用
 */
...
export async function performanceTrackingExample() {
    console.log("=== 性能追踪示例 ===");
    // 方式1: 使用全局性能追踪
    const result1 = await perf.trackAsync("database-query", async () => {
        // 模拟数据库查询
    ...
    const processResult = await componentLogger.trackAsync("data-processing", async () => {
        // 模拟数据处理
    ...
    // 同步性能追踪
    const syncResult = componentLogger.trackSync("sync-operation", () => {
        // 模拟同步计算
...
export async function apiAndDatabaseExample() {
    ...
    // 慢查询检测
    database.slowQuery("SELECT u.*, p.* FROM users u LEFT JOIN profiles p ON u.id = p.user_id WHERE u.department_id IN (SELECT id FROM departments WHERE region = ?)", 1250, // 执行时间
    1000, // 阈值
    {
        params: ["华北地区"],
        resultCount: 3456,
...
export function loggingAnalysisExample() {
    ...
    console.log(`导出大小 - JSON: ${jsonLogs.length}字符, CSV: ${csvLogs.length}字符, TXT: ${txtLogs.length}字符`);
    // 导出详细的诊断报告
...
export async function lowcodeEngineExample() {
    ...
    // 代码生成性能追踪
    const generationResult = await pluginLogger.trackAsync("component-generation", async () => {
        // 模拟组件生成过程
