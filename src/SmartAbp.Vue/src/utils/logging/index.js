 ...
 * 创建应用程序日志器
 */
export function createAppLogger(appName) {
    return logger.child({
        app: appName,
        source: "application",
    });
...
export const log = {
    ...
    fatal: (message, error, data) => logger.fatal(message, error, data),
};
/**
 * 性能追踪的便捷方法
 */
export const perf = {
    start: (name, context) => logger.startTimer(name, context),
    trackAsync: (name, operation, context) => logger.trackAsync(name, operation, context),
    trackSync: (name, operation, context) => logger.trackSync(name, operation, context),
};
/**
 * 系统级日志方法
 */
...
export const api = {
    ...
    error: (url, method, error, details) => {
...
export const database = {
    ...
    slowQuery: (sql, duration, threshold = 1000, details) => {
 ...
 * Vue组合式函数：使用性能追踪
 */
export function usePerformanceTracking(context) {
    const perfLogger = context ? logger.child(context) : logger;
    return {
        startTimer: (name) => perfLogger.startTimer(name, context),
        trackAsync: (name, operation) => perfLogger.trackAsync(name, operation, context),
        trackSync: (name, operation) => perfLogger.trackSync(name, operation, context),
    };
}
// ============= 默认初始化 =============
// 在模块加载时进行基本初始化
