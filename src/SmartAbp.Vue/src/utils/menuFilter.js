...
export class MenuPermissionFilter {
    constructor() {
        // 超级管理员用户名列表
        Object.defineProperty(this, "SUPER_ADMIN_USERS", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: ["admin", "admin666"]
        });
     ...
     * 超级管理员拥有访问所有菜单的权限 (*)
     */
    canAccessMenuItem(menuItem, currentUser) {
        // 超级管理员拥有所有权限 (*)
     ...
     * 递归处理文件夹菜单的子菜单
     */
    filterMenus(menus, currentUser, options = {}) {
     ...
     * 用于路由守卫检查
     */
    getAccessiblePaths(menus, currentUser) {
        const paths = [];
     ...
     * 基于菜单权限配置
     */
    canAccessPath(path, menus, currentUser) {
     ...
     * 普通用户只展开基础功能菜单
     */
    getDefaultExpandedMenuKeys(menus, currentUser) {
        const expandedKeys = [];
        const isSuperAdmin = this.isSuperAdmin(currentUser);
        const userRoles = currentUser?.roles || [];
        const isAdmin = userRoles.includes("admin");
        const collectExpandedKeys = (menuItems) => {
            menuItems.forEach((menu) => {
     ...
     * 用于设置菜单激活状态
     */
    findParentMenu(menus, targetKey) {
