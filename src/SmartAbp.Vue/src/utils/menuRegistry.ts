...
import type { MenuItem, FolderMenuItem } from '@/types/menu'
...
export class MenuRegistry {
   ...
   * 搜索菜单（支持模糊搜索）
   */
  search(keyword: string): MenuItem[] {
   ...
   * 在子菜单中查找
   */
  private findInChildren(children: MenuItem[], path: string): MenuItem | null {
   ...
   * 从扁平列表构建树结构
   */
  private buildTreeFromFlat(menus: MenuItem[]): MenuItem[] {
    // 按order排序
    ...
    roots.forEach(root => {
      if (root.type === 'folder') {
        const folderRoot = root as FolderMenuItem
   ...
   * 检查菜单是否有父级
   */
  private hasParent(menu: MenuItem, allMenus: MenuItem[]): boolean {
