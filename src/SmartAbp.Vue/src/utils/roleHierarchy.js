 ...
 */
export function hasRolePermission(userRoles, requiredRoles) {
    // 没有角色限制，直接通过
    ...
    // 计算用户的最高角色权重
    const userMaxWeight = Math.max(...userRoles.map(role => getRoleWeight(role)));
    // 计算所需的最低角色权重
    const requiredMinWeight = Math.min(...requiredRoles.map(role => getRoleWeight(role)));
    // 用户最高权重 >= 所需最低权重，则通过
