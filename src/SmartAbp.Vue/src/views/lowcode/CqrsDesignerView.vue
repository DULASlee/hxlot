...
import { Document, CircleCheck, Download } from '@element-plus/icons-vue'
import {
  cqrsGeneratorApi,
...
function handleDownload() {
  ...
  // For now, download as a single JSON file
  const dataStr = JSON.stringify(generationResult.value.files, null, 2)
  const dataBlob = new Blob([dataStr], { type: 'application/json' })
  const url = URL.createObjectURL(dataBlob)
  const link = document.createElement('a')
  link.href = url
  link.download = `${generationResult.value.moduleName}_CQRS_${Date.now()}.json`
  link.click()
