...
import { SecurityMetrics, SecurityAlert, AbnormalBehavior, ComplianceIssue } from '../../packages/lowcode-designer/src/types/security'
...
vi.mock('element-plus', () => ({
  ...
  ElCard: { name: 'ElCard', template: '<div class="el-card"><slot name="header"></slot><slot></slot></div>' },
...
vi.mock('../../src/composables/useRealTimeAlerts', () => ({
  useRealTimeAlerts: () => ({
    ...
    connectionStatus: ref({ connected: false, connecting: false, lastConnected: null, reconnectAttempts: 0, error: null }),
    unreadCount: ref(0)
  })
}))
...
describe('SecurityDashboard', () => {
  ...
  describe('Performance Optimization', () => {
    ...
    it('should implement virtual scrolling for large alert lists', async () => {
      ...
      mockActiveAlerts.value = Array(1000).fill(null).map((_, i) => ({
        id: i.toString(),
        type: 'HighRiskPermissionAccess',
        severity: 'Medium',
        description: `Alert ${i}`,
