...
const codeLineTracker_1 = require("./codeLineTracker");
...
class AIGuardianExtension {
    constructor(context) {
        ...
        // v2.0新增：创建代码行数状态栏
        this.lineCountStatusBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 99 // 稍低优先级，显示在AI状态栏右边
        );
        ...
        // v2.0新增：初始化代码行数追踪器
        this.codeLineTracker = new codeLineTracker_1.EnhancedCodeLineTracker({
...
上次引擎检查: ${lastCheck}`;
    ...
    setupCodeLineTracking() {
        this.context.subscriptions.push(vscode.workspace.onDidChangeTextDocument((event) => {
            ...
            if (linesAdded > 0) {
                this.codeLineTracker.addCode(event.document.fileName, event.contentChanges.map(c => c.text).join('\n'));
                // 更新状态和状态栏
     ...
     * 更新行数状态栏
     */
    updateLineCountStatusBar(stats) {
     ...
     * 获取下一个检查点
     */
    getNextCheckpoint(currentLines) {
        if (currentLines < 100)
            return 100;
        if (currentLines < 200)
            return 200;
     ...
     * 100行审查点处理
     */
    async handle100LinesReached() {
        this.log('📊 已达到100行审查点');
        const stats = this.codeLineTracker.getSessionStats();
        const action = await vscode.window.showInformationMessage(`📊 100行代码审查点\n\n` +
            `已编写 ${stats.totalLines} 行代码\n` +
            `创建 ${stats.filesCount} 个文件\n\n` +
            `💡 建议：\n` +
            `• 代码方向是否正确？\n` +
            `• 是否有重复代码？\n` +
            `• 是否需要重构？`, { modal: false }, '继续编写', '查看报告', '暂停审查');
        if (action === '查看报告') {
            this.showLineReport();
        }
        else if (action === '暂停审查') {
     ...
     * 200行审查点处理
     */
    async handle200LinesReached() {
        this.log('⚠️ 已达到200行审查点');
        const stats = this.codeLineTracker.getSessionStats();
        const action = await vscode.window.showWarningMessage(`⚠️ 200行深度审查点！\n\n` +
            `已编写 ${stats.totalLines} 行代码\n` +
            `创建 ${stats.filesCount} 个文件\n` +
            ...
            `• 立即执行质量门禁（推荐）\n` +
            `• 补充单元测试\n` +
            `• 添加文档注释`, { modal: true }, '立即执行质量门禁', '继续到300行', '查看报告');
        if (action === '立即执行质量门禁') {
     ...
     * 280行警告处理
     */
    handle280LinesWarning() {
        this.log('🚨 280行警告');
        vscode.window.showWarningMessage(`🚨 警告：已编写${this.aiState.currentSessionLines}行，接近300行限制！\n\n` +
            `请准备执行质量门禁检查。`, '知道了');
    }
    /**
     * 300行强制停止处理
     */
    async handle300LinesForceStop() {
        this.log('🛑 已达到300行限制，强制触发质量门禁');
        const action = await vscode.window.showErrorMessage(`🛑 已达到300行代码限制！\n\n` +
            ...
            `3. 架构合规检查\n` +
            `4. 功能验证\n\n` +
            `点击"执行质量检查"开始验证。`, { modal: true }, '执行质量检查', '查看详情');
        if (action === '执行质量检查') {
            await this.runFullQualityGate();
        }
        else if (action === '查看详情') {
    ...
    async runFullQualityGate() {
        ...
        await vscode.window.withProgress({
            ...
            if (allPassed) {
                vscode.window.showInformationMessage('✅ 质量门禁检查全部通过！\n可以继续编写代码。');
                // 重置计数器
                this.codeLineTracker.resetSession();
                ...
                const failedChecks = Object.entries(results)
                    ...
                    .map(([name, _]) => name)
                    .join(', ');
                vscode.window.showErrorMessage(`❌ 质量门禁检查未通过！\n失败项: ${failedChecks}\n\n请修复问题后再继续。`, '查看日志').then(action => {
                    if (action === '查看日志') {
                        this.outputChannel.show();
                    }
                });
    ...
    async runTypeScriptCheck() {
        try {
            ...
            const { stdout, stderr } = await execAsync('cd src/SmartAbp.Vue && npm run type-check', { cwd: workspaceRoot, timeout: 60000 });
    ...
    async runESLintCheck() {
        try {
            ...
            const { stdout, stderr } = await execAsync('cd src/SmartAbp.Vue && npm run lint', { cwd: workspaceRoot, timeout: 60000 });
            this.log('✅ ESLint检查通过');
            return true;
        }
        catch (error) {
    ...
    async runArchitectureCheck() {
        try {
            ...
            // 检查相对路径违规
            const { stdout: relativePathCheck } = await execAsync('grep -r "\'\\.\\./\'" src/SmartAbp.Vue/packages/ | wc -l', { cwd: workspaceRoot });
            // 检查主应用引用违规
            const { stdout: mainAppCheck } = await execAsync('grep -r "@/" src/SmartAbp.Vue/packages/ | grep -v node_modules | wc -l', { cwd: workspaceRoot });
