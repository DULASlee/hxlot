...
describe('AI Guardian 集成功能测试', function () {
    ...
    describe('对话框关闭功能测试', function () {
        it('应该能够执行closeDialogsAndModals方法', async function () {
            ...
            try {
                // 执行对话框关闭功能
                await extension.closeDialogsAndModals();
                // 验证执行的命令
                ...
                for (const expectedCmd of expectedCommands) {
                    assert.ok(executedCommands.includes(expectedCmd), `应该执行命令: ${expectedCmd}`);
                }
        ...
        it('应该能够发送ESC键序列', async function () {
            ...
            try {
                await extension.sendEscapeKeySequence();
                // 验证ESC键相关命令
                const escCommands = [
                    'workbench.action.acceptSelectedSuggestion',
                    'workbench.action.closeQuickOpen',
                ...
                for (const cmd of escCommands) {
                    assert.ok(executedCommands.includes(cmd), `应该执行ESC键命令: ${cmd}`);
                }
    ...
    describe('智能恢复消息发送测试', function () {
        it('应该能够发送恢复消息（阶段1）', async function () {
            ...
            try {
                const result = await extension.smartSendRecoveryMessage(1);
                // 验证结果
        ...
        it('应该能够发送恢复消息（阶段2）', async function () {
            ...
            try {
                const result = await extension.smartSendRecoveryMessage(2);
                assert.strictEqual(result, true, '应该成功发送恢复消息');
    ...
    describe('新会话开启测试', function () {
        it('应该能够开启新聊天会话', async function () {
            ...
            try {
                const result = await extension.openNewChatSession();
                ...
                assert.ok(executedCommands.some(cmd => expectedCommands.includes(cmd)), '应该执行新会话命令');
    ...
    describe('连接状态检测测试', function () {
        it('应该能够检测AI连接状态', function () {
            // 设置最近活动时间
            extension.aiState.lastActivity = Date.now() - 10000; // 10秒前
            const isConnected = extension.isAIConnected();
        ...
        it('应该能够检测AI断开状态', function () {
            // 设置很久以前的活动时间
            extension.aiState.lastActivity = Date.now() - 60000; // 60秒前
            const isConnected = extension.isAIConnected();
    ...
    describe('等待连接测试', function () {
        it('应该能够等待连接（模拟快速连接）', async function () {
            let checkCount = 0;
            // 模拟AI连接检测
            const originalIsAIConnected = extension.isAIConnected;
            extension.isAIConnected = function () {
                checkCount++;
                // 第3次检查时返回true（模拟连接成功）
                return checkCount >= 3;
            };
            try {
                ...
                const result = await extension.waitForConnection(15);
                ...
                extension.isAIConnected = originalIsAIConnected;
            }
        });
        it('应该能够处理连接超时', async function () {
            ...
            try {
                const startTime = Date.now();
                const result = await extension.waitForConnection(3); // 3秒超时
                const duration = Date.now() - startTime;
    ...
    describe('智能三级恢复策略测试', function () {
        it('应该能够执行完整的三级恢复策略', async function () {
            ...
            let sentMessages = [];
            // 模拟方法调用
            const originalCloseDialogsAndModals = extension.closeDialogsAndModals;
            const originalSmartSendRecoveryMessage = extension.smartSendRecoveryMessage;
            const originalOpenNewChatSession = extension.openNewChatSession;
            const originalWaitForConnection = extension.waitForConnection;
            extension.closeDialogsAndModals = async () => {
                executedCommands.push('closeDialogsAndModals');
                return Promise.resolve();
            };
            extension.smartSendRecoveryMessage = async (phase) => {
                if (phase === 1) {
                    phase1Attempts++;
                    // 第2次尝试成功
            ...
            extension.openNewChatSession = async () => {
                executedCommands.push('openNewChatSession');
                return true;
            };
            extension.waitForConnection = async (timeout) => {
                executedCommands.push(`waitForConnection(${timeout})`);
                // 模拟连接成功
                return true;
            };
            try {
                await extension.attemptDirectReconnect();
                // 验证执行流程
                ...
                // 恢复原始方法
                extension.closeDialogsAndModals = originalCloseDialogsAndModals;
                extension.smartSendRecoveryMessage = originalSmartSendRecoveryMessage;
                extension.openNewChatSession = originalOpenNewChatSession;
                extension.waitForConnection = originalWaitForConnection;
            }
        });
    });
    describe('恢复上下文构建测试', function () {
        it('应该能够构建智能恢复上下文', function () {
            // 设置工作上下文
            extension.aiState.lastWorkContext = '测试任务：实现用户管理功能';
            const context = extension.buildRecoveryContext();
        ...
        it('应该能够处理空的工作上下文', function () {
            // 清空工作上下文
            extension.aiState.lastWorkContext = '';
            const context = extension.buildRecoveryContext();
