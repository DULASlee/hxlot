 ...
 * 模拟恢复上下文构建逻辑
 */
function buildRecoveryContext(state: MockAIState): string {
 ...
 * 模拟重启命令生成逻辑
 */
function generateRestartCommand(platform: string, execPath: string): string {
  if (platform === 'win32') {
...
suite('AI Guardian - 恢复逻辑核心测试', () => {
  ...
  suite('状态管理逻辑测试', () => {
    test('离线状态应正确标识', () => {
      const state: MockAIState = {
        lastActivity: Date.now() - 100 * 1000, // 100秒前
        isOnline: true,
        activityCount: 10
      };
    ...
    test('在线状态应正确标识', () => {
      const state: MockAIState = {
        lastActivity: Date.now() - 30 * 1000, // 30秒前
        isOnline: true,
        activityCount: 10
      };
