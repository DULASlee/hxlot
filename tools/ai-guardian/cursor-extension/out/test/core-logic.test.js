    ...
    async testEngineResponse() {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                const hasEngineResponse = engineKeywords.some(keyword => mockResponse.includes(keyword));
...
describe('AI Guardian 核心逻辑测试', function () {
    ...
    describe('AI状态监控功能', function () {
        ...
        it('应该能够检测AI离线状态', async function () {
            // 模拟AI长时间无活动
            const state = guardian.getState();
            guardian.aiState.lastActivity = Date.now() - 11 * 60 * 1000; // 11分钟前
    ...
    describe('执行引擎检查功能', function () {
        ...
        it('应该能够测试引擎响应', async function () {
            const result = await guardian.testEngineResponse();
    ...
    describe('状态栏显示功能', function () {
        ...
        it('应该正确生成离线状态文本', function () {
            guardian.aiState.isOnline = false;
            const statusText = guardian.generateStatusText();
    ...
    describe('完整工作流程测试', function () {
        ...
        it('应该能够处理离线恢复场景', async function () {
            // 1. 模拟离线状态
            guardian.aiState.isOnline = false;
            guardian.aiState.lastActivity = Date.now() - 15 * 60 * 1000; // 15分钟前
            // 2. 检查离线状态
            const isOffline = await guardian.isAIOffline();
...
//# sourceMappingURL=core-logic.test.js.map