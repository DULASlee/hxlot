...
export class AIGuardianExtension {
  ...
  private onConfigChange() {
    ...
    if (this.engineCheckTimer) clearInterval(this.engineCheckTimer);
  ...
  private async testWithLanguageModelAPI(): Promise<boolean | null> {
    try {
      // 检查是否有可用的语言模型
      ...
      for (let i = 0; i < maxRetries; i++) {
        try {
          ...
          // 检查响应是否包含执行引擎启动标志
          const engineKeywords = this.config.get<string[]>('engineResponseKeywords', [
            'AI编程铁律自动执行引擎已启动',
            '专家模式已激活',
            '九重爆雷',
            '编程前强制学习',
  ...
  private async testEngineFiles(): Promise<boolean> {
    try {
      // 备用方案：检查执行引擎相关文件和活动
      const workspaceFolders = vscode.workspace.workspaceFolders;
  ...
  private async promptLoadEngine() {
    const action = await vscode.window.showWarningMessage(
      '⚠️ AI编程铁律执行引擎未加载！建议立即加载以确保代码质量。',
      '立即加载',
      '发送加载指令',
      '30分钟后再检查'
    );
  ...
  public async loadExecutionEngine() {
    try {
      // 方法1: 尝试打开执行引擎规则文件
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (workspaceFolders) {
  ...
  private async sendLoadEngineCommand() {
...
`;
  ...
  private async restartCursorIDE(): Promise<void> {
    ...
    if (platform === 'win32') {
      ...
      try {
        exec(`osascript -e 'quit app "Cursor"' && sleep 2 && open -a Cursor`, (error) => {
          if (error) {
      ...
      try {
        exec(`killall cursor && sleep 2 && cursor &`, (error) => {
          if (error) {
            this.log(`⚠️ [Level 3] Restart command error: ${error}`);
          }
        });
