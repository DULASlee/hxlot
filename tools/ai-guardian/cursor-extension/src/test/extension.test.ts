...
import { AIGuardianExtension, AIState } from '../extension';
...
const mockVscode = {
  ...
  workspace: {
    ...
    openTextDocument: (path: string) => Promise.resolve({
      uri: { fsPath: path },
      fileName: path,
      languageId: 'markdown'
    })
...
suite('AI Guardian Extension Tests', () => {
  ...
  suite('AI状态监控功能测试', () => {
  test('应该正确初始化AI状态', () => {
    const initialState = (extension as any).aiState as AIState;
    ...
    test('应该能够记录AI活动', () => {
      ...
      const state = (extension as any).aiState as AIState;
    ...
    test('应该能够检测AI离线状态', async () => {
      ...
      const isOffline = await (extension as any).isAIOffline();
  ...
  suite('执行引擎检查功能测试', () => {
    ...
    test('应该能够测试引擎文件存在性', async () => {
      const result = await (extension as any).testEngineFiles();
  ...
  suite('Language Model API交互功能测试', () => {
    test('应该能够与Language Model API交互', async () => {
      const result = await (extension as any).testWithLanguageModelAPI();
      assert.strictEqual(result, true, 'Language Model API交互应该成功');
    });

    test('应该能够检测AI响应中的执行引擎标志', async () => {
      const result = await (extension as any).testEngineResponse();
  ...
  suite('自动恢复机制测试', () => {
    ...
    test('应该能够自动恢复', async () => {
      ...
      await (extension as any).autoRecover();
    ...
    test('应该能够发送恢复消息', async () => {
      await (extension as any).sendRecoveryMessage();
  ...
  suite('状态栏更新功能测试', () => {
    test('应该能够更新状态栏显示', () => {
      ...
      (extension as any).updateStatusBar();
    ...
    test('应该根据AI状态显示不同的状态栏文本', () => {
      ...
      extension.recordActivity();
      (extension as any).updateStatusBar();
      let statusBarItem = (extension as any).statusBarItem;
      ...
      state.isOnline = false;
      (extension as any).updateStatusBar();
      statusBarItem = (extension as any).statusBarItem;
  ...
  suite('生命周期管理测试', () => {
    test('应该能够启动监控', () => {
      ...
      // 验证定时器被创建
      const monitorTimer = (extension as any).monitorTimer;
      const engineCheckTimer = (extension as any).engineCheckTimer;
...
suite('集成测试', () => {
  test('完整工作流程测试', async () => {
    ...
    const extension = new AIGuardianExtension(mockContext);
